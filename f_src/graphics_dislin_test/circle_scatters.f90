...
!
  ...
  real ( kind = 8 ) t
  real ( kind = 8 ) x(n_circle)
  real ( kind = 8 ), allocatable :: xy_in(:,:)
  real ( kind = 8 ), allocatable :: xy_out(:,:)
  real ( kind = 8 ) y(n_circle)
...
!
  do i = 1, n_circle
    t = 0.5D+00 * pi * real ( i - 1, kind = 8 ) / real ( n_circle - 1, kind = 8 )
    x(i) = cos ( t )
    y(i) = sin ( t )
  end do
  call thkcrv ( 10 )
