...
!  Input:
!
!    integer ( kind = 4 ) N, the number of preassigned nodes.
!
!    real ( kind = 8 ) PNODES(N), the preassigned nodes.
!
!    integer ( kind = 4 ) IWORK(N), working array.
!
!    real ( kind = 8 ) WORK(N,N+1).
!
!    integer ( kind = 4 ) LDW, the number of elements in the 
...
!  Output:
!
!    real ( kind = 8 ) T(0:N), the coefficients of the polynomial whose
...
!  Input:
!
!    integer ( kind = 4 ) N, the number of nodes in the quadrature rule.
!    1 <= N.
!
!    real ( kind = 8 ) PNODES(N), the nodes.
!
!    real ( kind = 8 ) WT(N), the weights.
!
!    integer ( kind = 4 ) K, the index of the orthogonal polynomial whose
!    weighted square is to be integrated.  0 <= K.
!
!    real ( kind = 8 ) H0, the integral of the orthogonality weight
...
!  Output:
!
!    real ( kind = 8 ) TEST, the approximate value of the test integral
