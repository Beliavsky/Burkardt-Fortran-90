...
!
!    integer ( kind = 4 ) N, the number of time steps to take, plus 1. 
!
!    real ( kind = 8 ) D, the diffusion coefficient.  
!
!    real ( kind = 8 ) T, the total time.
!
!    integer ( kind = 4 ) SEED, a seed for the random number generator.
!
!  Output:
!
!    real ( kind = 8 ) X(M,N), the successive locations of the particle.
!
!    integer ( kind = 4 ) SEED, a seed for the random number generator.
!
  implicit none
