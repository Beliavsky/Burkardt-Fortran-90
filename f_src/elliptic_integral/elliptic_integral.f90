...
!  Input:
!
!    integer ( kind = 4 ) N_DATA.  The user sets N_DATA to 0 before the first call.  
!
!  Output:
!
!    integer ( kind = 4 ) N_DATA.  On each call, the routine increments N_DATA by 1,
!    and returns the corresponding data; when there is no more data, the
!    output value of N_DATA will be 0 again.
!
!    real ( kind = 8 ) U, the argument of the function.
!
!    real ( kind = 8 ) A, K, M, the parameters of the function.
!
