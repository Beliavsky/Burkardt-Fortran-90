...
!
  ...
  call fft_accuracy_check ( )
  call fft_speed_check ( nits )
!
!  Terminate.
!
  write ( *, '(a)' ) ' '
...
subroutine fft_accuracy_check ( )

!*****************************************************************************80
!
!! fft_accuracy_check compares the recovered data to the original.
!
!  Discussion:
!
!    Because the transform is not normalized, if we have perfect accuracy,
!    then a transformation followed by a back transformation should simply
!    multiply the data by N.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license.
...
!
    ...
    allocate ( z(1:2*n) )
!
!  Save the initial data in z.
!
    do i = 1, 2 * n
      z(i) = ggl ( seed )
    end do
!
!  Copy the initial data into X.
!
    x(1:2*n) = z(1:2*n)
!
...
! 
!  Transform is not normalized, so presumably X = N * Z.
!
    x(1:2*n) = x(1:2*n) / n
...
subroutine fft_speed_check ( nits )

!*****************************************************************************80
!
!! fft_speed_check estimates the speed of fft.
!
!  Discussion:
!
!    The complex data in an N vector is stored as pairs of values in a
...
!
    ...
    write ( *, '(2x,i12,2x,i8,2x,g12.4,2x,g12.4,2x,g12.4)' ) &
      n, nits, wtime, wtime / real ( 2 * nits, kind = 8 ), mflops
