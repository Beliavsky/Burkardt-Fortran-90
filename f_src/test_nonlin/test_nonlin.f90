...
!  Output:
!
!    real ( kind = 8 ) FJAC(N,N), the approximante jacobian matrix.
!
  implicit none
...
!  Input:
!
!    integer ( kind = 4 ) PROBLEM, the number of the problem.
!
!    integer ( kind = 4 ) N, the number of variables and equations.
!
!    real ( kind = 8 ) X(N), the evaluation point.
...
!  Output:
!
!    real ( kind = 8 ) FJAC(N,N), the jacobian matrix.
!
  implicit none
...
!  Output:
!
!    integer ( kind = 4 ) PROBLEM_NUM, the number of problems available.
!
  implicit none
...
!    integer ( kind = 4 ) PROBLEM, the problem number.
!
!    integer ( kind = 4 ) N, the order of the system.  N is primarily
...
!
!    real ( kind = 8 ) X(N), a solution value, if IKNOW is positive.
!
  implicit none
...
!  Input:
!
!    integer ( kind = 4 ) PROBLEM, is the problem number.
!
!    integer ( kind = 4 ) N, is the number of variables and equations.
!
!  Output:
!
!    real ( kind = 8 ) X(N), the starting point.
!
  implicit none
...
!  Local:
!
!    real ( kind = 8 ) C, the constant, which must be between 0 and 1.
!
  implicit none
...
!
!    real ( kind = 8 ) A(N), the vector whose 2-norm is desired.
!
!  Output:
!
!    real ( kind = 8 ) R8VEC_NORM2, the 2-norm of A.
!
  implicit none
...
!    N must be positive.
!
!    real ( kind = 8 ) A_LU(N,N), the LU factors from R8GE_FA.
!
!    integer ( kind = 4 ) PIVOT(N), the pivot vector from R8GE_FA.
!
