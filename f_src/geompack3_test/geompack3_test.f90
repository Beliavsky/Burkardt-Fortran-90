...
!*****************************************************************************80
!
!! GEOMPACK3_TEST tests geompack3().
!
!  Discussion:
!
!    "stop" statements replaced by "stop 1" so that UNIX can detect the
!    error status.
!
!    This program includes a few operational test routines, and many others
!    that are not operational.  The nonoperational routines are simply not
!    called.
!
!    The nonoperational routines would need various input files or 
!    interactive inputs in order to be executed.
!
!  Modified:
!
!    25 January 2021
...
!
  ...
  hoflag = (nfhol > 0)
  nfhol = max(nfhol,0)
  aspc2d = aspc2d* r8_pi ( ) /180.0D+00
  atol2d = atol2d* r8_pi ( ) /180.0D+00
  angacc = angacc* r8_pi ( ) /180.0D+00
  nfph = nface + nfhol
  ...
  if ( nfph >= maxfp ) then
    write ( *, 610) 'maxfp',nfph+1
    stop 1
  end if

  if ( npolh >= maxhf) then
    write ( *, 610) 'maxhf',npolh+1
    stop 1
  end if
...
!
  ...
  nrfe = 0
  angmin = 2.0D+00 * r8_pi ( )
  do i = 1,nvert
    if (eang(i) > r8_pi ( ) + tol) nrfe = nrfe + 1
    if (eang(i) > -1.0D+00) angmin = min(angmin,eang(i))
  end do

  angmin = angmin*180.0D+00/ r8_pi ( )
...
!
  ...
  angspc = angspc* r8_pi ( ) /180.0D+00
  angtol = angtol* r8_pi ( ) /180.0D+00
...
!
  ...
  angmin = 2.0D+00 * r8_pi ( )
  do i = 1,nvert
    if (iang(i) > r8_pi ( ) + tol) nrfv = nrfv + 1
...
!
  ...
  do while ( .true. )
   ...
   write ( *, '(a)' ) 'enter case 1,2,3,4,5 or 0 to exit:'
   read ( *, * ) case

   if ( case < 1 .or. 5 < case ) then
     exit
   end if
...
!
  ...
  write ( *, '(a)' ) ' '
  write ( *, '(a,i3)' ) '  Vector after applying a shift of ', shift
  write ( *, '(a)' ) ' '
...
!
  ...
  real ( kind = 8 ) samin2
  real ( kind = 8 ) sang(4)
  real ( kind = 8 ) sangmn,xc,xc0,xc1,xcinc,xd,xd0,xd1,xdinc
      ...
      do while ( xd <= xd1 )
        ...
        do while ( yd <= yd1 )
          ...
          do while ( zd <= zd1 )
            ...
            samin1 = min(sang(1),sang(2),sang(3),sang(4)) - r8_pi ( )
   ...
   if (case == 1) then
      ...
      if (radsq >= 0.0D+00) radsq = sqrt(radsq)
       ...
       read (irdr,*) a(1),a(2),a(3),b(1),b(2),b(3),c(1),c(2),c(3), &
           d(1),d(2),d(3)
       call sdang(a,b,c,d,sang,dang)
       write ( *, 600) 'sang=',(sang(i) - r8_pi ( ),i=1,4)
