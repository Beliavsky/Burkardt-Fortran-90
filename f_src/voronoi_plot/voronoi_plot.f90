...
!    and makes an ASCII PPM file plot of the Voronoi regions.
!
...
!
!    P3
!    # feep.ppm created by PBMLIB(PPMA_WRITE).
!    4 4
!    15
...
!    Input, integer ( kind = 4 ) NROW, NCOL, the number of rows and columns.
!
!    Input, integer ( kind = 4 ) RGB_MAX, the maximum value of any data
!    component.
!
!    Output, integer ( kind = 4 ) IERROR, an error flag.
!    0, no error.
...
!
!  Points very near the generator are black.
!
      if ( generator_dist <= 7.0D+00 * psize ) then
        ...
        b(i,j) = 0
!
!  Points closest to the generator get the generator's color.
!
      else
