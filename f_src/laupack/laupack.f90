...
!  Input:
!
!    integer ( kind = 4 ) NNODE, the number of nodes.
!
!    integer ( kind = 4 ) NEDGE, the number of edges.
!
!    integer ( kind = 4 ) INODE(NEDGE), JNODE(NEDGE); INODE(I) and
!    JNODE(I) are the end nodes of the I-th edge in the input graph.
!
!  Output:
!
!    integer ( kind = 4 ) NUMCYC, the number of independent cycles 
!    in the graph.
!
!    integer ( kind = 4 ) NUMCMP, the number of components of the graph.
!
!    integer ( kind = 4 ) NCYC(NNODE); each time the output WRITE 
