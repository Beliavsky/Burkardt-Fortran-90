...
!  Parameters:
!
!    Output, real ( kind = 8 ) DISK01_QUARTER_AREA, the area.
!
  implicit none
  ...
  disk01_quarter_area = 0.25D+00 * r8_pi * r * r

  return
end
subroutine disk01_quarter_monomial_integral ( e, integral )
...
!
  ...
  arg = real ( e(1) + 3, kind = 8 ) / 2.0D+00
  f1 = r8_gamma ( arg )
  arg = real ( e(2) + 1, kind = 8 ) / 2.0D+00
  f2 = r8_gamma ( arg )
  arg = real ( e(1) + e(2) + 4, kind = 8 ) / 2.0D+00
  f3 = r8_gamma ( arg )

  integral = f1 * f2 / f3 / 2.0D+00 / real ( 1 + e(1), kind = 8 )

  return
end
subroutine disk01_quarter_sample ( n, seed, x )

!*****************************************************************************80
!
!! DISK01_QUARTER_SAMPLE uniformly samples points from the unit quarter disk.
!
!  Licensing:
!
!    This code is distributed under the GNU LGPL license. 
