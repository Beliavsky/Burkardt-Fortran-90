...
!
!    Input, integer ( kind = 4 ) NS(M), the number of points along 
!    each dimension.
!
!    Input, real ( kind = 8 ) A(M), B(M), the endpoints for each dimension.
!
!    Input, integer ( kind = 4 ) C(M), the grid centering for each dimension.
!    1 <= C(*) <= 5.
!
!    Output, real ( kind = 8 ) X(M,N) = X(M*S(1),S(2),...,S(M)), the points.
!
  implicit none
