...
!
!    Input, integer ( kind = 4 ) NS(2), the number of points along 
!    each dimension.
!
!    Input, real ( kind = 8 ) A(2), B(2), the endpoints for each dimension.
!
!    Input, integer ( kind = 4 ) C(2), the grid centering for each dimension.
!    1 <= C(*) <= 5.
!
!    Output, real ( kind = 8 ) X(2,N) = X(2,S(1)*S(2)), the points.
!
  implicit none
